...
router.get("/getuserbyid/:id", async (req, res) => {
    try {
        let DB = await readDB()
        let user = DB.find((x:any) => x.id == req.params.id);
        console.log(user);
...
router.get("/findtask/:id", async (req, res) => {
    try {
        ...
        const alltask = users.flatMap((u:any) => u.task || []);

        let find = alltask.find((x:any) => x.id === id)
...
router.delete("/deletetaskbyid/:id", async (req, res) => {
    try {
        ...
        let userIndex = users.findIndex((u:any) => u.task?.some((t:any) => t.uid === taskId));
        ...
        users[userIndex].task = users[userIndex].task.filter((t:any) => t.uid !== taskId);
...
router.delete("/deleteuser/:id", async (req, res) => {
    try {
        ...
        let newDB = DB.filter((x:any) => x.id !== req.params.id)
        await writeDB(newDB)
